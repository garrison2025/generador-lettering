...
// Dynamically import heavy components (未被使用，可以考虑移除，但暂时保留以保持与原始代码一致)
...
export default function EditorClient() {
  ...
  useEffect(() => {
    if (plantillaId) {
      ...
      if (plantilla) {
        ...
        setColor(plantilla.color); setAlignment(plantilla.alignment);
  ...
  const saveDesign = () => { toast({ title: "Función no disponible", description: "Esta funcionalidad requiere un sistema de cuentas de usuario.", variant: "destructive", }) }
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl">
            Crea diseños tipográficos personalizados con nuestro editor de lettering online. Personaliza fuentes, colores, efectos y más para crear lettering único para cualquier ocasión.
          </p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
          <div>
            ...
            {!isMobile && ( <div className="mt-4"> <Card> <CardContent className="p-4"> <h3 className="font-bold mb-2">Plantillas de Lettering Populares</h3> <div className="grid grid-cols-2 gap-2">{PLANTILLAS.slice(0, 4).map((plantilla) => ( <Button key={plantilla.id} variant="outline" size="sm" className="h-auto py-2 justify-start" onClick={() => { setText(plantilla.texto); setFont(plantilla.estilo); setFontSize(plantilla.fontSize); setColor(plantilla.color); setAlignment(plantilla.alignment); setLetterSpacing(plantilla.letterSpacing); setLineHeight(plantilla.lineHeight); setRotation(plantilla.rotation); setShadow(plantilla.shadow); if (plantilla.shadow) { setShadowColor(plantilla.shadowColor!); setShadowBlur(plantilla.shadowBlur!); setShadowOffsetX(plantilla.shadowOffsetX!); setShadowOffsetY(plantilla.shadowOffsetY!); } setOutline(plantilla.outline); if (plantilla.outline) { setOutlineColor(plantilla.outlineColor!); setOutlineWidth(plantilla.outlineWidth!); }}}> {plantilla.nombre} </Button> ))} </div> <Button variant="link" size="sm" className="mt-2 w-full" asChild><Link href="/plantillas">Ver todas las plantillas de lettering</Link></Button> </CardContent> </Card> </div>)}
          </div>
          <div className="space-y-4">
              ...
              <CardContent className="p-6">
                <div className="mb-4 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <Popover> <PopoverTrigger asChild><Button variant="outline" size="sm" disabled={isExporting}>{isExporting ? (<Loader2 className="h-4 w-4 animate-spin mr-2" />) : (<Download className="h-4 w-4 mr-2" />)}Exportar</Button></PopoverTrigger> <PopoverContent className="w-56" align="end"><div className="grid gap-2"><Button variant="outline" size="sm" onClick={() => exportAsImage("png")} className="justify-start" disabled={isExporting}>PNG (Transparente)</Button><Button variant="outline" size="sm" onClick={() => exportAsImage("jpg")} className="justify-start" disabled={isExporting}>JPG (Fondo Blanco)</Button></div></PopoverContent> </Popover>
                  </div>
                </div>
            ...
            {!isMobile && ( <Card> <CardContent className="p-6"> <h3 className="font-bold mb-4">Consejos para un Lettering Perfecto</h3> <ul className="list-disc pl-5 space-y-2 text-muted-foreground"> <li>Experimenta con diferentes combinaciones de fuentes y colores para encontrar tu estilo.</li> <li>Usa sombras sutiles para dar profundidad a tu texto.</li> <li>Ajusta el espaciado entre letras para mejorar la legibilidad.</li> <li>Prueba a rotar ligeramente el texto para darle un toque dinámico.</li> <li>Combina diferentes tamaños de texto para crear jerarquía visual.</li> </ul> </CardContent> </Card>)}
          </div>
        </div>
