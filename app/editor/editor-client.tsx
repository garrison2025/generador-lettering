...
import { TouchColorPicker } from "@/components/ui/touch-color-picker"; // <--- 1. 导入 TouchColorPicker
...
export default function EditorClient() {
  ...
  const [color, setColor] = useState("#5B4FBE"); // color state
  ...
  useEffect(() => { console.log("STAGE 2c + ColorPicker: Font loading useEffect - DOM manipulation COMMENTED OUT."); }, []);
  ...
  const textStyle: React.CSSProperties = {
    fontFamily: currentFont.family, fontSize: `${fontSize}px`, color: color, // Uses color state
  ...
  console.log("STAGE 2c + MINIMAL TouchColorPicker TEST: Adding just TouchColorPicker.");
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
          <div>
            <Tabs defaultValue="texto" className="w-full">
              ...
              {/* 2. 在 "Estilo" 选项卡中添加 TouchColorPicker */}
              <TabsContent value="estilo" className="space-y-4 mt-4">
                <div className="space-y-2">
                  <Label htmlFor="color-select-test">Color del Texto (Test)</Label>
                  <TouchColorPicker
                    value={color} // 使用现有的 color state
                    onChange={setColor} // 使用现有的 setColor
                    presetColors={COLORS.map((c) => c.value)} // 使用 COLORS 常量
                    // className="mt-1" // 如果原始代码有 className，可以加上
                  />
                </div>
                {/* 其他 "Estilo" 选项卡的 TouchSlider 和 Button 暂时不加 */}
                <p style={{color:"navy", fontWeight:"bold"}}>ONLY TouchColorPicker added to Estilo Tab.</p>
              </TabsContent>
          ...
          <div style={{border: '1px solid lightskyblue', padding: '10px', background: '#f0f8ff'}}>
            <h3 className="text-lg font-semibold mb-2">Vista Previa (Stage 2c + ColorPicker)</h3>
        ...
        <p style={{color: 'darkviolet', marginTop: '20px', textAlign: 'center', fontWeight: 'bold'}}>STAGE 2c + MINIMAL TouchColorPicker TEST: Added only TouchColorPicker to "Estilo" tab.</p>
      </main>
      <SiteFooter />
    </div>
  )
