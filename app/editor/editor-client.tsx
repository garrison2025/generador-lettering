...
// import dynamic from "next/dynamic" // 暂时不用 dynamic import
import { Button } from "@/components/ui/button" // 保留，因为原始代码有，可能后续用到
...
import Link from "next/link" // 保留
import { Card, CardContent } from "@/components/ui/card" // 保留
import { Label } from "@/components/ui/label" // 保留
// import { Switch } from "@/components/ui/switch" // Stage 2
import { Textarea } from "@/components/ui/textarea" // Stage 2 (TextControls),但这里先用原生textarea
...
export default function EditorClient() {
  ...
  // 从URL加载模板
  useEffect(() => {
    console.log("STAGE 1: Plantilla loading useEffect, plantillaId:", plantillaId);
    if (plantillaId) {
  ...
  console.log("STAGE 1: Rendering. Text:", text, "Font:", currentFont.name);

  // exportAsImage 和 saveDesign 函数暂时不加，因为它们依赖 html2canvas 和 toast，
  // 并且 exportAsImage 还操作 previewRef.current (DOM)
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
          {/* 控制面板 - 先只放一个简单的文本输入 */}
          <div style={{border: '1px solid lightgray', padding: '10px', background: '#f0f0f0'}}>
            <h3 className="text-lg font-semibold mb-2">Controles (Stage 1)</h3>
            <div>
              <Label htmlFor="textInputBasic">Texto:</Label>
              <textarea // 使用原生 textarea 进行测试
                id="textInputBasic"
                value={text}
        ...
        <p style={{color: 'green', marginTop: '20px', textAlign: 'center'}}>Stage 1 Test: Basic states, constants, useEffects (DOM op commented), text input & preview. SEO block restored.</p>
      </main>
      <SiteFooter />
    </div>
  )
