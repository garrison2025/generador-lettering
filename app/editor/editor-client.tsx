...
// Definición de fuentes, colores y plantillas (sin cambios)
const FONTS = [
...
export default function EditorClient() {
  ...
  const previewRef = useRef<HTMLDivElement>(null) // Para escritorio
  const mobilePreviewRef = useRef<HTMLDivElement>(null) // Para móvil

  // Carga de fuentes (sin cambios)
  useEffect(() => {
  ...
  // Carga de plantilla desde URL (sin cambios)
  useEffect(() => {
    if (plantillaId) {
  ...
  // Exportar como imagen - MODIFICADO
  const exportAsImage = async (type: "png" | "jpg") => {
    // Determinar qué elemento capturar basado en si es móvil o no
    const elementToCapture = isMobile ? mobilePreviewRef.current : previewRef.current

    if (!elementToCapture) {
      console.error("Error: El elemento de vista previa no se encontró para la exportación.")
      toast({
        title: "Error al exportar",
        description: "No se pudo encontrar el elemento de vista previa. Por favor, inténtalo de nuevo.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      const canvas = await html2canvas(elementToCapture, options) // Usar el elemento determinado
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="border rounded-lg p-4 bg-[#F4F4F8]">
                  {isMobile ? (
                    // Pasar la ref a MobileLetteringPreview
                    // Asegúrate de que MobileLetteringPreview pueda aceptar y usar esta ref
                    // Puede que necesites usar React.forwardRef en MobileLetteringPreview
                    <MobileLetteringPreview
                      ref={mobilePreviewRef} // <-- MODIFICACIÓN AQUÍ
                ...
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setFontSize(Math.floor(Math.random() * 50) + 40)
