...
// ... (所有其他 import 保持不变, 确保 TouchSlider 和 TouchColorPicker 都已导入)
...
export default function EditorClient() {
  ...
  const [letterSpacing, setLetterSpacing] = useState(0); const [lineHeight, setLineHeight] = useState(1.5); // state for line height
  ...
  useEffect(() => { console.log("STAGE 2d (LineHeight Slider): Font loading useEffect - DOM manipulation COMMENTED OUT."); }, []);
  ...
  console.log("STAGE 2d (LineHeight Slider): Testing TouchSlider for Line Height.");
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
          <div>
            <Tabs defaultValue="texto" className="w-full">
              ...
              <TabsContent value="estilo" className="space-y-4 mt-4">
                ...
                {/* 1. 添加用于“行高”的 TouchSlider */}
                <TouchSlider
                  label="Altura de Línea"
                  min={0.8}
                  max={3}
                  step={0.1}
                  value={lineHeight} // 使用 lineHeight state
                  onChange={setLineHeight} // 使用 setLineHeight
                />

                {/* 其他 "Estilo" 选项卡的 Rotation Slider 和 Button 暂时不加 */}
                <p style={{color:"purple", fontWeight:"bold"}}>Line Height Slider added. Rotation Slider & Button still pending.</p>
              </TabsContent>
          ...
          <div style={{border: '1px solid lightskyblue', padding: '10px', background: '#f0f8ff'}}>
            <h3 className="text-lg font-semibold mb-2">Vista Previa (Stage 2d - Line Height)</h3>
            <div style={textStyle}> {text || "Escribe algo..."} </div>
          </div>
          {!isMobile && ( /* SEO 内容块 */ )}
        </div>
        <p style={{color: 'darkmagenta', marginTop: '20px', textAlign: 'center', fontWeight: 'bold'}}>STAGE 2d TEST (Adding Line Height Slider): Checking for build/runtime errors.</p>
      </main>
      <SiteFooter />
    </div>
  )
